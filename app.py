    ...
    def generate_report(self, task_id, report_type, app_name, services, incident_params=None):
        ...
        try:
            ...
            task_progress[task_id].update(
                {"progress": 10, "message": "Setting up report system..."}
            )
            ...
            task_progress[task_id].update(
                {"progress": 20, "message": "Testing AppDynamics connection..."}
            )
            ...
            if connection_success:
                task_progress[task_id].update(
                    {"progress": 30, "message": "Connected to AppDynamics successfully"}
                )
            else:
                task_progress[task_id].update(
                    {"progress": 30, "message": "Using demo data (AppDynamics unavailable)"}
                )

            # Generate report based on type
            if report_type == "performance":
                task_progress[task_id].update(
                    {"progress": 50, "message": "Generating performance analysis report..."}
                )
                ...
                if analysis_scope == "service" and target_service:
                    ...
                    report_paths = self.sre_system.generate_full_report_suite(
                        application_name=app_name, services=services
                    )

            elif report_type == "incident":
                task_progress[task_id].update(
                    {"progress": 50, "message": "Generating incident analysis report..."}
                )
...
def api_generate_report():
    ...
    try:
        ...
        return jsonify({"task_id": task_id, "message": "Report generation started"})
...
def api_task_status(task_id):
    ...
    status = task_progress.get(
        task_id, {"status": "not_found", "progress": 0, "message": "Task not found"}
    )
...
def api_appdynamics_applications():
    ...
    try:
        ...
        if not collector.test_connection():
            return jsonify(
                {"success": False, "error": "Unable to connect to AppDynamics", "applications": []}
            )
        ...
        return jsonify({"success": True, "applications": app_list, "count": len(app_list)})
...
def readiness_check():
    """Readiness check endpoint - verifies app is ready to serve requests"""
    try:
        ...
        if reports_dir.exists():
            components["reports_directory"] = "ready"
        else:
            components["reports_directory"] = "creating"
        ...
        return (
            jsonify(
                {"status": "not_ready", "error": str(e), "timestamp": datetime.utcnow().isoformat()}
            ),
            503,
        )
...
def liveness_check():
    """Liveness check endpoint - verifies app is alive (not deadlocked)"""
