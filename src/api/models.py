...
class MetricValue(BaseModel):
    ...
    error_budget_consumed: float = Field(..., description="Error budget consumed (%)", ge=0, le=100)
...
class ServiceHealth(BaseResponse):
    ...
    service_name: str = Field(..., description="Service name")
    health_score: float = Field(..., description="Health score (0-100)", ge=0, le=100)
    status: ServiceStatus = Field(..., description="Overall status")
    metrics_count: int = Field(..., description="Number of metrics analyzed")
    slo_compliance: float = Field(..., description="SLO compliance percentage", ge=0, le=100)
    insights: List[str] = Field(
        default_factory=list, description="Health insights and observations"
    )
...
class BreachPrediction(BaseModel):
    ...
    forecast_time: Optional[datetime] = Field(None, description="When breach is predicted to occur")
...
class AnomalyReport(BaseResponse):
    ...
    anomalies: List[AnomalyDetail] = Field(default_factory=list, description="Detected anomalies")
    baseline_health: str = Field(..., description="Baseline health status")
    prediction: Optional[BreachPrediction] = Field(None, description="SLO breach prediction")
    recommendations: List[str] = Field(
        default_factory=list, description="Actionable recommendations"
    )
    detection_method: DetectionMethod = Field(
        DetectionMethod.MODIFIED_Z_SCORE, description="Detection method used"
    )
...
class CreateReportRequest(BaseModel):
    ...
    services: List[str] = Field(..., description="Services to include in report", min_items=1)
