...
from src.exceptions import FileWriteError, PDFGenerationError, TemplateError
...
class PDFReportGenerator:
    ...
    def _log_capabilities(self) -> None:
        ...
        self.logger.info(
            f"  🌐 Browser PDF (Puppeteer): {'✅ Available' if BROWSER_PDF_AVAILABLE else '❌ Not Available'}"
        )
        self.logger.info(
            f"  📄 WeasyPrint: {'✅ Available' if WEASYPRINT_AVAILABLE else '❌ Not Available'}"
        )
        self.logger.info(
            f"  📋 ReportLab: {'✅ Available' if REPORTLAB_AVAILABLE else '❌ Not Available'}"
        )
    ...
    def _create_weasyprint_pdf(self, html_content: str, output_path: str) -> str:
        ...
        success = pdf_generator.create_pdf_from_html(
            html_content, output_path, base_url=f"file://{Path().absolute()}/"
        )
    ...
    def _create_reportlab_pdf(
        self, metrics: List[SLOMetric], incident: Optional[IncidentData], output_path: str
    ) -> str:
        """
        ...
        try:
            ...
            summary_data = [
                ...
                [
                    "Compliant Metrics",
                    f"{summary['compliant_count']} ({summary['compliance_percentage']:.1f}%)",
                ],
            ...
            metrics_table = Table(
                metrics_data,
                colWidths=[1.5 * inch, 1.5 * inch, 1 * inch, 1 * inch, 1 * inch, 1 * inch],
            )
            ...
            if trend_chart_images:
                ...
                for chart_name, image_path in trend_chart_images.items():
                    try:
                        ...
                        chart_image = ReportLabImage(
                            image_path, width=6.5 * inch, height=4.5 * inch
                        )
            ...
            raise PDFGenerationError(
                f"ReportLab PDF generation failed: {str(e)}", context={"output_path": output_path}
            )
    ...
    def _generate_recommendations(self, summary: Dict[str, Any]) -> List[str]:
        ...
        if summary["breached_count"] > 0:
            recommendations.append(
                f"• URGENT: Address {summary['breached_count']} SLO breach(es) immediately"
            )
        if summary["at_risk_count"] > 0:
            recommendations.append(
                f"• Monitor {summary['at_risk_count']} service(s) at risk of SLO breach"
            )
    ...
    def _add_incident_to_pdf(self, story: List, incident: IncidentData, styles) -> None:
        ...
        incident_data = [
            ...
            [
                "End Time",
                incident.end_time.strftime("%Y-%m-%d %H:%M:%S") if incident.end_time else "Ongoing",
            ],
    ...
    def _optimize_template_for_pdf(self, html_content: str) -> str:
        ...
        for pattern, replacement in pdf_optimizations:
            html_content = re.sub(
                pattern, replacement, html_content, flags=re.DOTALL | re.IGNORECASE
            )
