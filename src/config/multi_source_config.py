...
class ConfigurationManager:
    ...
    def _create_default_config(self) -> MultiSourceConfig:
        ...
        data_sources = [
            ...
            DataSourceConfig(
                ...
                authentication={"bearer_token": "${PROMETHEUS_TOKEN}"},
                polling_interval=60,
                enabled=False,
            ),
            DataSourceConfig(
                ...
                connection_params={"file_path": "data/sample_metrics.csv", "file_type": "csv"},
    ...
    def get_slo_targets(self, service_name: str) -> List[SLOTarget]:
        ...
        targets_data = self.config.analytics.default_slo_targets.get(
            service_name, self.config.analytics.default_slo_targets.get("default", [])
        )
            ...
            if ds.source_type == DataSourceType.APPDYNAMICS:
                ...
                for param in required_params:
                    if param not in ds.connection_params:
                        issues.append(
                            f"AppDynamics source '{ds.name}' missing required parameter: {param}"
                        )
